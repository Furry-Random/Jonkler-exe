...
photos = "Photos.exe" #armazena o nome do app padrão pra abrir imagens jpg do Windows

#tente fechar o jonkler.exe
try:
    ...
    print(f"O {jonkler} foi derrotado!")

    #tente fechar o Photos.exe
    try:

        #fecha o "Photos.exe" por meio do "taskkill", (comando do CMD)
        subprocess.run(f"taskkill /F /IM {photos}", check=True, shell=True, capture_output=True)

        #diz pro usuário que o jonkler foi derrotado e agora ele está a salvo
        print(f"E todas as imagens do {jonkler} foram destruídas, mas você precisa fechar as abas no navegador manualmente...")
        time.sleep(10)
        print("E nem me adianta me olhar com essa cara, eu já fiz quase todo o trabalho sujo!")
        time.sleep(7)

    #erro ao fechar o Photos.exe
    except subprocess.CalledProcessError as e:
        
        #caso o "jonkler.exe" não esteja aberto ou o "Batman.exe" tenha dado um erro, (por sei lá o motivo)
        print(f"Mas eu não consegui destruir as imagens do {jonkler}, talvez nenhuma imagem estava aberta...")
        time.sleep(7)
        
        #isso é pro usuário conseguir ler o resultado e abrir o link no terminal
        print("E nem me adianta me olhar com essa cara, eu já derrotei o jonkler!")
        time.sleep(7)

#erro ao fechar o jonkler.exe
except subprocess.CalledProcessError as e:
